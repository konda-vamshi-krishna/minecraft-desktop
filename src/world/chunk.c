...
// ponytail: self-contained Raylib matrix multiplication avoiding header collision or missing raymath.h
static inline Matrix Matrix_Multiply(Matrix left, Matrix right) {
    Matrix result = { 0 };
...
void World_Render(const Camera* camera, float renderAlpha) {
    ...
    // Compute Model-View-Projection matrix from current Raylib 3D camera (Raylib convention: MV * P)
    Matrix matModelView = rlGetMatrixModelview();
    Matrix matProjection = rlGetMatrixProjection();
    Matrix matMVP = Matrix_Multiply(matModelView, matProjection);
