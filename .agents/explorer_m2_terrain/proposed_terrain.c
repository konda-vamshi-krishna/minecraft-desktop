/**
 * @file terrain.c
 * @brief Milestone 2: Procedural World Generation Engine Implementation.
 *
 * Implements multi-octave 2D Simplex terrain with fBM, Whittaker biomes,
 * 3D volumetric density with overhangs, dual 3D Simplex cave carve-out,
 * and deterministic cellular feature stamping with zero cross-chunk mutation.
 */

#include "proposed_terrain.h"
#include <math.h>
#include <string.h>

/* ========================================================================= */
/* 1. Mathematical Constants & Gradient Tables                               */
/* ========================================================================= */
...
/* Fast floor for coordinate conversion */
static inline int FastFloor(float x) {
    int ix = (int)x;
    return (x < (float)ix) ? (ix - 1) : ix;
}

/* Y-internal index formula: stride 1 along vertical columns for cacheline locality */
static inline int GetVoxelIndex(int lx, int ly, int lz) {
...
void Terrain_GenerateChunk(int chunkX, int chunkZ, uint64_t worldSeed, uint8_t* outVoxels) {
    ...
    memset(outVoxels, BLOCK_AIR, CHUNK_TOTAL_VOXELS);

    float heights[CHUNK_SIZE_X][CHUNK_SIZE_Z];
    BiomeType biomes[CHUNK_SIZE_X][CHUNK_SIZE_Z];

    /* Step 1: Precompute 2D Continental Heightmap & Biomes across 16x16 grid */
    for (int lz = 0; lz < CHUNK_SIZE_Z; lz++) {
        int wz = chunkZ * CHUNK_SIZE_Z + lz;
        for (int lx = 0; lx < CHUNK_SIZE_X; lx++) {
            int wx = chunkX * CHUNK_SIZE_X + lx;
    ...
    for (int lz = 0; lz < CHUNK_SIZE_Z; lz++) {
        for (int lx = 0; lx < CHUNK_SIZE_X; lx++) {
            ...
            for (int ly = CHUNK_SIZE_Y - 1; ly >= 1; ly--) {
                if (outVoxels[GetVoxelIndex(lx, ly, lz)] == BLOCK_STONE) {
                    surfaceY = ly;
                    break;
                }
            }
            ...
            if (surfaceY > TERRAIN_SEA_LEVEL) {
                ...
                if (biome == BIOME_DESERT) {
                    outVoxels[GetVoxelIndex(lx, surfaceY, lz)] = BLOCK_SAND;
                    for (int dy = 1; dy <= 3; dy++) {
                        if (surfaceY - dy >= 5 && outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] == BLOCK_STONE) {
                            outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] = BLOCK_SAND;
                        }
                    }
                    for (int dy = 4; dy <= 6; dy++) {
                        if (surfaceY - dy >= 5 && outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] == BLOCK_STONE) {
                            outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] = BLOCK_SANDSTONE;
                        }
                    }
                } else if (biome == BIOME_MOUNTAINS) {
                    if (surfaceY > TERRAIN_SNOW_LINE) {
                        outVoxels[GetVoxelIndex(lx, surfaceY, lz)] = BLOCK_SNOW;
                    }
                    /* Lower mountain rock remains BLOCK_STONE */
                } else {
                    /* Plains & Forest: Topsoil Grass + Subsurface Dirt */
                    outVoxels[GetVoxelIndex(lx, surfaceY, lz)] = BLOCK_GRASS;
                    for (int dy = 1; dy <= 3; dy++) {
                        if (surfaceY - dy >= 5 && outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] == BLOCK_STONE) {
                            outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] = BLOCK_DIRT;
                        }
                    }
                }
            } else {
                ...
                for (int dy = 0; dy <= 2; dy++) {
                    if (surfaceY - dy >= 5 && outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] == BLOCK_STONE) {
                        outVoxels[GetVoxelIndex(lx, surfaceY - dy, lz)] = (biome == BIOME_DESERT) ? BLOCK_SAND : BLOCK_DIRT;
                    }
                }
            }
        }
    ...
    BiomeType centerBiome = biomes[8][8];
    ...
    for (int t = 0; t < treeCount; t++) {
        ...
        int ty = -1;
        for (int y = CHUNK_SIZE_Y - 6; y > TERRAIN_SEA_LEVEL; y--) {
            if (outVoxels[GetVoxelIndex(tx, y, tz)] == BLOCK_GRASS) {
                ty = y;
                break;
            }
        }

        if (ty > TERRAIN_SEA_LEVEL && ty + 8 < CHUNK_SIZE_Y) {
            uint64_t rh = Terrain_SplitMix64Next(&state);
            int th = 4 + (int)(rh % 3ULL); /* Height in [4, 6] */

            /* Trunk */
            outVoxels[GetVoxelIndex(tx, ty, tz)] = BLOCK_DIRT;
            for (int y = ty + 1; y <= ty + th; y++) {
                outVoxels[GetVoxelIndex(tx, y, tz)] = BLOCK_WOOD;
            }
            ...
            for (int dy = th - 1; dy <= th; dy++) {
                ...
                for (int dx = -2; dx <= 2; dx++) {
                    for (int dz = -2; dz <= 2; dz++) {
                        ...
                        int lx = tx + dx;
                        int lz = tz + dz;
                        int idx = GetVoxelIndex(lx, cy, lz);
            ...
            for (int dx = -1; dx <= 1; dx++) {
                for (int dz = -1; dz <= 1; dz++) {
                    int lx = tx + dx;
                    int lz = tz + dz;
                    int idx = GetVoxelIndex(lx, cy1, lz);
                    if (outVoxels[idx] == BLOCK_AIR) {
                        outVoxels[idx] = BLOCK_LEAVES;
                    }
                }
            ...
            for (int c = 0; c < 5; c++) {
                ...
                int idx = GetVoxelIndex(lx, cy2, lz);
                if (outVoxels[idx] == BLOCK_AIR) {
                    outVoxels[idx] = BLOCK_LEAVES;
                }
            }
    ...
    if (centerBiome == BIOME_DESERT) {
        ...
        for (int c = 0; c < cactusCount; c++) {
            ...
            for (int y = 200; y > TERRAIN_SEA_LEVEL; y--) {
                if (outVoxels[GetVoxelIndex(cx, y, cz)] == BLOCK_SAND) {
                    uint64_t rh = Terrain_SplitMix64Next(&state);
                    int ch = 1 + (int)(rh % 3ULL);
                    for (int cy = y + 1; cy <= y + ch && cy < CHUNK_SIZE_Y; cy++) {
                        outVoxels[GetVoxelIndex(cx, cy, cz)] = BLOCK_CACTUS;
                    }
                    break;
                }
            }
    ...
    if (centerBiome == BIOME_PLAINS || centerBiome == BIOME_FOREST) {
        ...
        for (int f = 0; f < flowerCount; f++) {
            ...
            for (int y = 250; y > TERRAIN_SEA_LEVEL; y--) {
                if (outVoxels[GetVoxelIndex(fx, y, fz)] == BLOCK_GRASS) {
                    int idxAbove = GetVoxelIndex(fx, y + 1, fz);
